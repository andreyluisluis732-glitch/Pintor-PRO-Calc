  ...
  const handleGetHelp = async () => {
    ...
    try {
      ...
      const response = await ai.getGenerativeModel({ model: "gemini-3-flash-preview" }).generateContent({
        contents: [{ role: 'user', parts: [{ text: `O usuário está com a seguinte dificuldade no aplicativo Pintor PRO Calc: "${difficulty}". 
      ...
      setAiResponse(response.response.text() || "Desculpe, não consegui gerar uma resposta no momento. Tente novamente.");
    } catch (error) {
